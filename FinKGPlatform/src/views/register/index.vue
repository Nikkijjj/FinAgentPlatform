    ...
    <div class="view-account-container animate__animated animate__fadeInDown">
      ...
      <div class="view-account-form">
        ...
        >
          ...
          <n-form-item class="default-color back-login-row">
            <div class="back-login-only">
...
<script setup lang="ts">
  import { websiteConfig } from '@/config/website.config';
  import { LockClosedOutline, PersonOutline, MailOutline } from '@vicons/ionicons5';
  import { onMounted, reactive, ref } from 'vue';
  import { FormItemRule, useMessage } from 'naive-ui';
  import { useRouter } from 'vue-router';
  import { useUserStore } from '@/store/modules/user';
    ...
    userInfoForm.value.validate(async (errors) => {
      if (!errors) {
        ...
        try {
          const params = {
            name: userInfo.name,
            email: userInfo.email, // 将account改为email
            password: userInfo.password,
            // user_investment_profile: mockEmptyUserInfo.user_investment_profile, // 如果后端不需要可以注释掉
          };

          console.log('注册参数:', params); // 调试用
    ...
    <div class="view-account-container animate__animated animate__fadeInDown">
      ...
      <div class="view-account-form">
        ...
        >
          ...
          <!-- 将account字段改为email字段 -->
          <n-form-item path="email" class="username-item">
            <n-input
