...
- 新增管理员专属“图谱构建质量报告”能力：
	- 构建页质量卡片改为仅管理员可见。
	- 在图谱构建流程上方新增“查看图谱构建报告”按钮，支持按 run_id 查看完整报告。
	- 报告弹层新增运行信息、核心质量指标、原始 JSON 明细。
- 新增管理员专属 AI 问答诊断能力：
	- 智能分析页新增诊断入口与抽屉。
	- 展示 `quality_hints`、`web_search_meta`、`retrieval_debug`。
	- 管理员请求自动附带 `debug_retrieval=true`，便于定位检索与联网补强问题。
- 新增质量报告接口适配与前端容错：
	- 新增按项目与 run 查询质量报告接口封装。
	- 对报告接口 404 场景增加降级策略与本地缓存回退，避免页面刷新后完全丢失报告上下文。

## 2.1.1

- 文档同步：`README.md` 按当前 FinAgentGraph 联调实现更新，补充流式问答接口说明。
- 纠正模板化描述：将 WebSocket 示例配置改为当前使用的流式接口路径示例。
