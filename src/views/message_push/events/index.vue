<script setup lang="ts">
    ...
    function getEventType(event: any): string {
      ...
      if (event.board_name || event.event_subtype?.includes('price') || event.event_subtype?.includes('leader')) {
        return 'industry';
      }
    ...
    function getEventSummary(item: any): string {
      ...
      switch(eventType) {
        ...
        case 'industry':
          ...
          switch(item.event_subtype) {
            ...
            case 'price_fall_abnormal':
              return `${boardName}板块跌幅异常，下跌${Math.abs(rawData.price_change || 0).toFixed(2)}%`;
        ...
        case 'trading':
          return item['标题']?.substring(0, 100) + '...' || '个股新闻';
    ...
    const columns_all: DataTableColumns<any> = [
      {
        ...
        filter(value, row) {
          console.log('过滤事件类型:', {
            filterValue: value,
            rowEventType: row.event_type,
            row_type: row.type,
            row_eventType: row.eventType,
            row_event_type: row._event_type,
            match: row._event_type === value
          });
          return row._event_type === value;
        },
        render(row) {
      ...
      },
      {
        ...
        render(row) {
          ...
          const config = levelMap[level] || { text: formatImpactLevel(level), class: '', color: '#666' };
      ...
      },
      {
        ...
        render(row) {
          ...
          const config = sentimentMap[sentiment] || { text: formatSentiment(sentiment), class: '', color: '#666' };
    ...
    function formatIndustryEventDetail(item: any) {
      ...
      switch(item.event_subtype) {
        ...
        case 'rise_consistency_abnormal':
          return `${boardName}板块上涨一致性异常：
          上涨家数：${rawData.rise_stocks || 0}/${rawData.total_stocks || 0}
          上涨占比：${rawData.rise_stocks && rawData.total_stocks ? ((rawData.rise_stocks / rawData.total_stocks) * 100).toFixed(1) : '0.0'}%
      ...
      <div v-if="paginationParams.total > 0" class="pagination-container">
        <n-space justify="center" align="center" class="pagination-wrapper">
          ...
          <span class="page-info">
            第 {{ paginationParams.page }} 页 / 共 {{ Math.ceil(paginationParams.total / paginationParams.size) }} 页
          </span>
          <n-button 
            :disabled="paginationParams.page >= Math.ceil(paginationParams.total / paginationParams.size)" 
            @click="loadData(paginationParams.page + 1)"
            size="small"
            secondary
          >
    ...
    <div v-if="currentDetail" class="detail-content">
      ...
      >
        ...
        <n-descriptions-item label="事件类型">
          <n-tag :type="currentDetail.event_type === 'macro' ? 'info' : 
                        currentDetail.event_type === 'industry' ? 'success' :
                        currentDetail.event_type === 'stock' ? 'warning' : 'error'">
        ...
        <!-- 详细内容 -->
        <n-descriptions-item v-if="currentDetail.event_type === 'stock' && currentDetail['内容']" label="完整内容">
        ...
        <n-descriptions-item v-if="currentDetail.event_type === 'macro' && currentDetail.raw_data?.新闻内容" label="新闻内容">
          <div style="max-height: 200px; overflow-y: auto; padding: 8px; background: #f8f9fa; border-radius: 4px;">
            <n-text style="white-space: pre-wrap; line-height: 1.6; font-size: 13px;">
        ...
        <!-- 股票相关信息 -->
        <n-descriptions-item v-if="currentDetail.symbol || currentDetail.event_type === 'stock'" label="相关标的">
          <n-text>
        ...
        <n-descriptions-item label="影响等级">
          <n-tag :type="currentDetail.impact_level === 'critical' ? 'error' : 
                         currentDetail.impact_level === 'high' ? 'warning' :
                         currentDetail.impact_level === 'medium' ? 'default' : 'info'">
            {{ formatImpactLevel(currentDetail.impact_level) }}
          </n-tag>
        </n-descriptions-item>

        <n-descriptions-item label="情感倾向">
          <n-tag :type="currentDetail.sentiment === 'positive' ? 'success' : 
                         currentDetail.sentiment === 'negative' ? 'error' : 'default'">
        ...
        <template v-if="currentDetail.raw_data">
          <!-- 价格数据 -->
          <n-descriptions-item v-if="currentDetail.raw_data.real_time_close !== undefined" label="实时价格">
          ...
          <n-descriptions-item v-if="currentDetail.raw_data.rise_stocks !== undefined" label="上涨家数">
            <n-text>
          ...
          <n-descriptions-item v-if="currentDetail.raw_data.leader_stock" label="领涨/跌股">
            <n-text>
              ...
              <span v-if="currentDetail.raw_data.leader_change !== undefined" 
